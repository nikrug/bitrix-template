...
function initializeLanguage() {
    ...
    // Если язык не поддерживается, устанавливаем по умолчанию
...
function updateTestimonialsText(lang) {
    // Получаем все элементы testimonials
    const testimonials = document.querySelectorAll('.testimonials__pannel');

    testimonials.forEach((panel, index) => {
        // Получение заголовка, подзаголовка и текста
        const title = panel.querySelector('.testimonials__pannel-title-text');
        const subtitle = panel.querySelector('.testimonials__pannel-subtitle-text');
        const text = panel.querySelector('.testimonials__text');

        // Поскольку индексация в langArr начинается с 1, при этом с index мы начинаем с 0,
        // используем (index + 1)
...
function startInterval() {
    intervalId = setInterval(() => {
        ...
        if (currentSlide > totalSlides) {
            ...
            // Убираем переход временно, чтобы избежать "прыжка"
            sliderWrapper.style.transition = 'none';
            sliderWrapper.style.transform = `translateX(0%)`;
            
            // используем таймаут, чтобы дождаться завершения применения стилей
            setTimeout(() => {
                currentSlide = 1; // Переход к первому слайду (дубликату)
